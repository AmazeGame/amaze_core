	...
	lists:foreach(
		fun
			({Listener,clear,TransportOpts,PackFrame})->
				{ok, _} = ranch:start_listener(Listener,ranch_tcp,TransportOpts,?MODULE,[]);
			({Listener,tls,TransportOpts,PackFrame})->
				{ok, _} = ranch:start_listener(Listener,ranch_ssl,TransportOpts,?MODULE,[])
		end,ListenerInfos),

	{ok, _} = ranch:start_listener(tcp_echo,
		ranch_tcp, [{port, 5555}], ?MODULE, []),
