...
unpack(PackBinary) ->
	case binary:split(PackBinary,?HEX_END_BINARY) of
		[_PackBinary0]->
			case catch binary_to_integer(PackBinary,16) of
				{'EXIT',_}-> {error,PackBinary};
				_Length  ->
					{?PARTITION_KEY,PackBinary}
			end;
		[PackBinary0,LeftBinary]->
			case catch binary_to_integer(PackBinary0,16) of
				{'EXIT',_}-> {error,PackBinary};
				Length ->
...
pack_length(Length) when is_integer(Length)->
	LenBinary = integer_to_binary(Length,16),
	<<LenBinary/binary,?HEX_END_CHAR>>.
